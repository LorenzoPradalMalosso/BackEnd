...
| RF-09 | Funcional | Gerenciamento de ativos patrimoniais | O sistema deve permitir cadastrar, visualizar, atualizar e excluir ativos patrimoniais da instituição. | Alta |
...
| RF-20 | Funcional | Relatórios de estoque | O sistema deve permitir visualizar relatórios de materiais em estoque e histórico de movimentações. | Alta |
| RF-21 | Funcional | Validação de estoque | O sistema deve impedir registrar saída de material quando a quantidade solicitada for maior que o estoque disponível. | Alta |
...
| RNF-07 | Não funcional | O sistema deve responder às requisições da API em tempo adequado para garantir boa experiência do usuário. | Alta |